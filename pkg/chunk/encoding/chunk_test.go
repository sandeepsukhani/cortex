...
func TestDeletedChunkIterator_Value(t *testing.T) {
	...
	chunkInterval := model.Interval{0, model.Time((numSamples - 1) * step)}
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			deletedIntervals: []model.Interval{
				{chunkInterval.Start, chunkInterval.Start.Add(10 * time.Minute)},
			},
		},
		{
			name: "delete last 10 minutes",
			deletedIntervals: []model.Interval{
				{chunkInterval.End.Add(-10 * time.Minute), chunkInterval.End},
			},
		},
		{
			name: "delete first 5 minutes and last 5 minutes",
			deletedIntervals: []model.Interval{
				{chunkInterval.Start, chunkInterval.Start.Add(5 * time.Minute)},
				{chunkInterval.End.Add(-5 * time.Minute), chunkInterval.End},
			},
		},
		{
			name: "delete in the middle",
			deletedIntervals: []model.Interval{
				{chunkInterval.Start.Add(5 * time.Minute), chunkInterval.End.Add(-5 * time.Minute)},
			},
		},
		{
...
func TestDeletedChunkIterator_Batch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			deletedIntervals: []model.Interval{
				{chunkInterval.Start, chunkInterval.Start.Add(time.Duration((BatchSize / 2) * step))},
			},
			seekAt: chunkInterval.Start.Add(11 * time.Minute),
		},
		{
			...
			deletedIntervals: []model.Interval{
				{chunkInterval.End.Add(-time.Duration((BatchSize / 2) * step)), chunkInterval.End},
			},
			seekAt: chunkInterval.Start.Add(11 * time.Minute),
		},
	} {
