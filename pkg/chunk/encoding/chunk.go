...
// finds index of deleted interval which starts immediately after the given time
func (dci *deletedChunkIterator) findNextDeletedIntervalIndex(t model.Time) int {
	for i := range dci.deletedIntervals {
		if dci.deletedIntervals[i].Start > t {
			return i
		}
